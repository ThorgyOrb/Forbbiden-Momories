...
public static class LibraryGodsSceneBuilder
{
    ...
    const string IconConfigPath = "Assets/Data/CardIconConfig.asset";
    ...
    public static void Build()
    {
        ...
        // La grilla deja libre una franja inferior para la barra de paginación: con el
        // catálogo completo solo se instancia una página de cartas, no las 14.651.
        var gridScrollArea = Region(gridArea, "GridScrollArea", new Vector2(0f, 0.065f), new Vector2(1f, 1f));
        var gridContent = MakeScrollGrid(gridScrollArea);

        var pagerBar = Region(gridArea, "PagerBar", new Vector2(0f, 0f), new Vector2(1f, 0.058f));

        var prevBtn = MakeButton(pagerBar, "BtnPrevPage", "‹ ANTERIOR", Violet, Bright);
        Place(prevBtn, new Vector2(0f, 0f), new Vector2(0.22f, 1f), Vector2.zero, Vector2.zero);

        var nextBtn = MakeButton(pagerBar, "BtnNextPage", "SIGUIENTE ›", Violet, Bright);
        Place(nextBtn, new Vector2(0.78f, 0f), new Vector2(1f, 1f), Vector2.zero, Vector2.zero);

        var pageLbl = Label(pagerBar, "PageLabel", "PÁGINA 1 / 1", 16, Bright,
                            TextAlignmentOptions.Center, FontStyles.Bold);
        Place(pageLbl, new Vector2(0.23f, 0f), new Vector2(0.77f, 1f), Vector2.zero, Vector2.zero);
        ...
        Set(ctrl, "nextPageButton", nextBtn);
        Set(ctrl, "pageLabel", pageLbl);
    ...
    static RectTransform BuildModel3DViewer(RectTransform root, out Model3DViewer viewer)
    {
        ...
        System.IO.Directory.CreateDirectory("Assets/Art/RenderTextures");
        var rt = new RenderTexture(900, 900, 16) { name = "LibraryGods_ViewerRT" };
        const string rtPath = "Assets/Art/RenderTextures/LibraryGods_ViewerRT.renderTexture";

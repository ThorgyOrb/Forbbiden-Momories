...
public class TestGrantAllCards : MonoBehaviour
{
    ...
    public void GrantAllCards()
    {
        ...
        // En lote: AddCopy guarda en cada llamada, y Save reescribe el archivo completo.
        // Sin esto son ~14.600 escrituras de un archivo que crece hasta 14.600 entradas y
        // la escena se cuelga. Con el lote se escribe UNA vez, al salir.
        col.BeginBatch();
        try
        {
            foreach (var card in LibraryCatalog.AllCards)
            {
                ...
                if (need > 0) { col.AddCopy(card.cardId, need); added += need; }
                cards++;
            }
        }
        finally
        {
            col.EndBatch();   // aquí ocurre el único guardado
        }

        sw.Stop();
        UpdateStatus($"¡Listo! {cards} cartas del catálogo a {CopiesPerCard} copias cada una " +
                     $"({added} copias añadidas) en {sw.ElapsedMilliseconds} ms.\n" +
                     "Guardado en la colección persistente.");
        Debug.Log($"TestGrantAllCards: {cards} cartas → {CopiesPerCard} copias c/u " +
                  $"({added} añadidas) en {sw.ElapsedMilliseconds} ms.");
    }

...
public class Library : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Start()
    {
        ...
        // 2. Instanciar por bloques, no una carta por cada CardData: con el catálogo
        //    completo (14.000+) instanciarlas todas de golpe congela el juego.
        _filler = new IncrementalGridFiller(gridContainer);
        _filler.Restart(_allCards.Length, SpawnCardRange);

        HidePreview();
    }

    /// <summary>Crea las cartas [from, from+count). Lo llama IncrementalGridFiller.</summary>
    private void SpawnCardRange(int from, int count)
    {
        for (int i = from; i < from + count && i < _allCards.Length; i++)
        {
            CardData data = _allCards[i];
